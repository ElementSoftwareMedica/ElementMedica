...
export function authenticate(options = {}) {
    ...
    return async (req, res, next) => {
        logger.info(`[AUTH_MIDDLEWARE] Request received: ${req.method} ${req.path}`);
        logger.info(`[AUTH_MIDDLEWARE] Original URL: ${req.originalUrl}`);
        logger.info(`[AUTH_MIDDLEWARE] Headers:`, req.headers);
        
        try {
            const token = extractToken(req);
            
            if (!token) {
                logger.info(`[AUTH_MIDDLEWARE] No token found, optional: ${optional}`);
                if (optional) {
                    req.person = null;
                    return next();
                }
