...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log('🚀 Form submit - identifier:', identifier);
    console.log('🚀 Form submit - identifier length:', identifier.length);
    console.log('🚀 Form submit - identifier chars:', identifier.split('').map((c, i) => `${i}: '${c}' (${c.charCodeAt(0)})`));

    try {
      await login(identifier, password);
      // Il redirect alla homepage avverrà automaticamente tramite il Navigate sopra
      // Non chiamiamo navigate() qui perché il componente si ri-renderizzerà con isAuthenticated=true
    } catch (err: any) {
