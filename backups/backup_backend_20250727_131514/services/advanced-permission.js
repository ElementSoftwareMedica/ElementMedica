...
class AdvancedPermissionService {
    ...
    async checkPermission(params) {
        ...
        try {
            ...
            // Recupera person con ruoli, compagnia e permessi avanzati
            ...
            // Verifica se è SUPER_ADMIN (accesso completo)
            ...
            // Cerca permessi avanzati specifici
            for (const role of person.personRoles) {
                for (const permission of role.advancedPermissions) {
                    // Verifica scope
                    const scopeCheck = this.checkScope(permission.scope, person, targetCompanyId);
                    if (!scopeCheck.allowed) {
                        continue;
                    }

                    // Verifica accesso per sede
                    const siteAccessCheck = this.checkSiteAccess(permission, person, targetSiteId);
                    if (!siteAccessCheck.allowed) {
                        continue;
                    }

                    // Verifica condizioni aggiuntive
                    const conditionCheck = this.checkConditions(permission.conditions, person, targetCompanyId);
                    if (!conditionCheck.allowed) {
                        continue;
                    }

                    // Filtra campi permessi
                    ...
                    return {
                        ...
                        siteAccess: permission.siteAccess,
                        siteId: permission.siteId,
                        reason: `Advanced permission granted for ${role.roleType}`
                    };
                }
            }

            // Fallback: verifica permessi base se non ci sono permessi avanzati
            const hasBasicPermission = await this.checkBasicPermission(person, resource, action);
            if (hasBasicPermission) {
                // Per ADMIN, concedi accesso con limitazioni di scope
                if (globalRole === 'ADMIN') {
                    return {
                        allowed: true,
                        allowedFields: this.getDefaultFieldsForRole(globalRole, resource),
                        scope: 'global',
                        reason: 'ADMIN basic permission'
                    };
                }

                // Per COMPANY_ADMIN, limita alla propria compagnia
                if (globalRole === 'COMPANY_ADMIN') {
                    const scopeCheck = this.checkScope('company', person, targetCompanyId);
                    if (scopeCheck.allowed) {
                        return {
                            allowed: true,
                            allowedFields: this.getDefaultFieldsForRole('COMPANY_ADMIN', resource),
                            scope: 'company',
                            reason: 'COMPANY_ADMIN basic permission'
                        };
                    }
                }
            }
