...
export const hasBackendPermission = (
  ...
  if (frontendPermission === 'persons:read' || frontendPermission === 'persons:view') {
    return backendPermissions['VIEW_PERSONS'] === true ||
           backendPermissions['PERSONS_VIEW'] === true ||
           backendPermissions['PERSONS:VIEW'] === true;
  }
  
  if (frontendPermission === 'persons:create') {
    return backendPermissions['CREATE_PERSONS'] === true ||
           backendPermissions['PERSONS_CREATE'] === true ||
           backendPermissions['PERSONS:CREATE'] === true;
  }
  
  if (frontendPermission === 'persons:edit' || frontendPermission === 'persons:update') {
    return backendPermissions['EDIT_PERSONS'] === true ||
           backendPermissions['PERSONS_EDIT'] === true ||
           backendPermissions['PERSONS:EDIT'] === true;
  }
  
  if (frontendPermission === 'persons:delete') {
    return backendPermissions['DELETE_PERSONS'] === true ||
           backendPermissions['PERSONS_DELETE'] === true ||
           backendPermissions['PERSONS:DELETE'] === true;
  }
  ...
  if (frontendPermission === 'persons:view_employees') {
    return backendPermissions['VIEW_EMPLOYEES'] === true ||
           backendPermissions['EMPLOYEES_VIEW'] === true ||
           backendPermissions['EMPLOYEES:VIEW'] === true;
  }
  
  if (frontendPermission === 'persons:view_trainers') {
    return backendPermissions['VIEW_TRAINERS'] === true ||
           backendPermissions['TRAINERS_VIEW'] === true ||
           backendPermissions['TRAINERS:VIEW'] === true;
  }
  ...
  if (frontendPermission === 'courses:read') {
    return backendPermissions['VIEW_COURSES'] === true ||
           backendPermissions['COURSES_VIEW'] === true ||
           backendPermissions['COURSES:VIEW'] === true;
  }
  ...
  if (frontendPermission === 'dashboard:read' || frontendPermission === 'dashboard:view') {
    return backendPermissions['VIEW_DASHBOARD'] === true || 
           ...
           backendPermissions['DASHBOARD_VIEW'] === true ||  // Formato invertito
           backendPermissions['DASHBOARD:VIEW'] === true ||  // Formato invertito con colon
           // Per admin, concedi accesso se ha permessi generali
           backendPermissions['ADMIN_PANEL'] === true ||
           backendPermissions['VIEW_COMPANIES'] === true ||
           backendPermissions['COMPANIES_VIEW'] === true ||
           backendPermissions['COMPANIES:VIEW'] === true; // Se può vedere aziende, può vedere dashboard
  }
  ...
  if (frontendPermission === 'companies:read' || frontendPermission === 'companies:view') {
    return backendPermissions['VIEW_COMPANIES'] === true || 
           ...
           backendPermissions['COMPANIES_VIEW'] === true ||  // Formato invertito
           backendPermissions['COMPANIES:VIEW'] === true;    // Formato invertito con colon
  }
...
export const convertBackendToFrontendPermissions = (
  ...
  Object.keys(backendPermissions).forEach(backendKey => {
    if (backendPermissions[backendKey] === true) {
      ...
      // Nuovi formati invertiti: COMPANIES_VIEW -> companies:read
      else if (backendKey.endsWith('_VIEW')) {
        const entity = backendKey.replace('_VIEW', '').toLowerCase();
        frontendPermissions[`${entity}:read`] = true;
        frontendPermissions[`${entity}:view`] = true;
      }
      else if (backendKey.endsWith('_CREATE')) {
        const entity = backendKey.replace('_CREATE', '').toLowerCase();
        frontendPermissions[`${entity}:create`] = true;
      }
      else if (backendKey.endsWith('_EDIT')) {
        const entity = backendKey.replace('_EDIT', '').toLowerCase();
        frontendPermissions[`${entity}:edit`] = true;
        frontendPermissions[`${entity}:update`] = true;
      }
      else if (backendKey.endsWith('_DELETE')) {
        const entity = backendKey.replace('_DELETE', '').toLowerCase();
        frontendPermissions[`${entity}:delete`] = true;
      }
      // Nuovo formato invertito con colon: COMPANIES:VIEW -> companies:read
      else if (backendKey.includes(':')) {
        const [maybeEntity, maybeAction] = backendKey.split(':');
        const actionUpper = (maybeAction || '').toUpperCase();
        const entityLower = (maybeEntity || '').toLowerCase();
        if (['VIEW','CREATE','EDIT','DELETE'].includes(actionUpper) && entityLower) {
          if (actionUpper === 'VIEW') {
            frontendPermissions[`${entityLower}:read`] = true;
            frontendPermissions[`${entityLower}:view`] = true;
          } else if (actionUpper === 'CREATE') {
            frontendPermissions[`${entityLower}:create`] = true;
          } else if (actionUpper === 'EDIT') {
            frontendPermissions[`${entityLower}:edit`] = true;
            frontendPermissions[`${entityLower}:update`] = true;
          } else if (actionUpper === 'DELETE') {
            frontendPermissions[`${entityLower}:delete`] = true;
          }
        }
      }
      ...
      else if (backendKey === 'VIEW_DASHBOARD' || backendKey === 'dashboard:read' || backendKey === 'dashboard:view' || backendKey === 'dashboard.view' || backendKey === 'dashboard.read' || backendKey === 'DASHBOARD_VIEW' || backendKey === 'DASHBOARD:VIEW') {
