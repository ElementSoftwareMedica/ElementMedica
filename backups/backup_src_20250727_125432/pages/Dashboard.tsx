...
const Dashboard: React.FC = () => {
  ...
  try {
    ...
    hasPermission = tenantContext.hasPermission;
  } catch (error) {
  ...
  // GDPR Consent Check
  const checkGdprConsent = useCallback(async () => {
    try {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [coursesData, trainersData, companiesData, employeesData, schedulesData] = await Promise.allSettled([
      ...
      const trainers = trainersData.status === 'fulfilled' ? trainersData.value : [];
      const companies = companiesData.status === 'fulfilled' ? companiesData.value : [];
  ...
  }, [tenant?.id, hasPermission, checkGdprConsent]);
