...
export function setupSecurity(app, options = {}) {
  ...
  if (enableHelmet) {
    const env = process.env.NODE_ENV || 'development';
    const baseCspDirectives = (CSP_CONFIG[env] && CSP_CONFIG[env].directives) ? CSP_CONFIG[env].directives : CSP_CONFIG.development.directives;
    const baseHsts = HSTS_CONFIG[env] || HSTS_CONFIG.development;
