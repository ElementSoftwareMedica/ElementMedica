...
const EmployeesSection: React.FC<EmployeesSectionProps> = ({ companyId }) => {
  ...
  const fetchEmployees = async () => {
    try {
      setLoading(true);
      const response = await apiGet(`/api/v1/persons?companyId=${companyId}&roleType=EMPLOYEE`) as { persons?: Employee[] };
      setEmployees(response.persons || []);
    } catch (error) {
      console.error('Error fetching employees:', error);
      showToast({ message: 'Errore nel caricamento dei dipendenti', type: 'error' });
    } finally {
      setLoading(false);
    }
  };
