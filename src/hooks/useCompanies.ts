...
export function useCompanies() {
  ...
  // P63: Usa operateTenantId per ricaricare quando cambia il tenant selezionato
  const { operateTenantId } = useTenantMode();
  ...
  // Initial load and reload when operateTenantId changes
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      loadCompanies(true); // Force refresh when tenant changes
    }, 100);
  ...
  }, [loadCompanies, operateTenantId]); // P63: Ricarica quando cambia operateTenantId
