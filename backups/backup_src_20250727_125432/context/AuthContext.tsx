...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (identifier: string, password: string) => {
    try {
      ...
      // La risposta del backend ha la struttura: {success, message, data: {accessToken, user, ...}}
      if (response.data?.accessToken) {
        console.log('💾 Saving token:', response.data.accessToken.substring(0, 20) + '...');
        ...
        if (response.data.user?.tenantId) {
          ...
          console.log('🏢 Saving tenant ID:', response.data.user.tenantId);
        }
        ...
        let mappedRole = 'User';
        if (response.data.user.roles?.includes('SUPER_ADMIN') || response.data.user.roles?.includes('ADMIN')) {
          mappedRole = 'Admin';
        } else if (response.data.user.roles?.includes('COMPANY_ADMIN')) {
          mappedRole = 'Administrator';
        }
        
        const mappedUser = {
          ...response.data.user,
          role: response.data.user.role || mappedRole // Use backend role if available, otherwise use mapped role
        };
