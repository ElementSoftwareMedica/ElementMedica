...
const SearchableDropdown: React.FC<SearchableDropdownProps> = ({
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        type="button"
        className={`w-full px-4 py-2 text-left bg-white border rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 flex items-center justify-between ${
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg">
          ...
          <div className="max-h-60 overflow-auto">
            ...
            <button
              type="button"
              className={`w-full px-4 py-2 text-left text-gray-500 hover:bg-gray-50 focus:outline-none focus:bg-gray-50 text-sm ${
                !value ? 'bg-orange-50 text-orange-600' : ''
              }`}
              onClick={() => {
                onChange('');
                setIsOpen(false);
