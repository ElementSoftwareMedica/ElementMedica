...
async function createTestUser(companyId, data = {}) {
  ...
  if (existing) {
    ...
    const requiredPerms = ['VIEW_EMPLOYEES', 'CREATE_EMPLOYEES', 'EDIT_EMPLOYEES', 'DELETE_EMPLOYEES'];
    ...
    const toCreate = requiredPerms.filter(p => !existingSet.has(p));
    if (toCreate.length > 0) {
      await prisma.rolePermission.createMany({
        data: toCreate.map(permission => ({ permission, personRoleId: adminRole.id }))
      });
    }
