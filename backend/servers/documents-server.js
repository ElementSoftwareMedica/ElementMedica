...
const envAllowedOriginsRaw = (process.env.ALLOWED_ORIGINS || process.env.CORS_ALLOWED_ORIGINS || process.env.FRONTEND_URL || '');
const envAllowedOrigins = envAllowedOriginsRaw
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
const DOCS_ALLOWED_ORIGINS = Array.from(new Set([...defaultAllowedOrigins, ...envAllowedOrigins]));

app.use(cors({
  origin: function (origin, callback) {
    // Consenti richieste senza origin (es. curl, mobile app)
    if (!origin) return callback(null, true);
    if (DOCS_ALLOWED_ORIGINS.includes(origin)) {
  ...
  allowedHeaders: [
    ...
    'x-tenant-id', 'X-Tenant-ID', 'cache-control', 'pragma', 'expires'
  ],
