...
router.get('/',
  ...
  [
    ...
    query('tipoServizio').optional().isIn(['CORSO', 'DVR', 'RSPP', 'MEDICO_COMPETENTE', 'PRIVACY', 'ALTRO']),
...
router.post('/',
  ...
  [
    body('tipoServizio').isIn(['CORSO', 'DVR', 'RSPP', 'MEDICO_COMPETENTE', 'PRIVACY', 'ALTRO']),
...
router.patch('/:id/stato',
  ...
  [
    param('id').isUUID(),
    body('nuovoStato').isIn(['BOZZA', 'INVIATO', 'VISUALIZZATO', 'ACCETTATO', 'RIFIUTATO', 'FATTURATO', 'ANNULLATO', 'ARCHIVIATO']),
    body('note').optional().isString()
  ],
  validate,
  async (req, res) => {
    try {
      ...
      // Traccia data visualizzazione
      if (nuovoStato === 'VISUALIZZATO' && !preventivo.dataVisualizzazione) {
        updateData.dataVisualizzazione = now;
      }
...
router.post('/:id/applica-sconto',
  authenticate,
  requirePermissions(['update:preventivi', 'read:codici_sconto']),
