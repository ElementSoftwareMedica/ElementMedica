import { RefreshCw, AlertCircle, CheckCircle2, Clock, Wifi } from 'lucide-react'
import { useSyncStatus } from '../sync/SyncStatusProvider'

export function SyncStatusBar(): JSX.Element {
    const { syncState, progress, lastSyncAt, lastDownloadAt, pendingOperations, conflicts } = useSyncStatus()
    ...
    return (
        <div className="flex items-center gap-3">
            ...
            {lastDownloadAt && (
                <div className="flex items-center gap-1.5" title={`Ultimo aggiornamento dati: ${new Date(lastDownloadAt).toLocaleString('it-IT')}`}>
                    <Wifi className="w-3 h-3 text-teal-500" />
                    <span className="text-[10px] text-gray-500">
                        Live: {new Date(lastDownloadAt).toLocaleTimeString('it-IT', { hour: '2-digit', minute: '2-digit' })}
                    </span>
                </div>
            )}
            {lastSyncAt && (
