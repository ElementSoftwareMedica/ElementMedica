/**
 * Script per aggiornare il template PREVENTIVO con layout v11
 * Basato su test-preventivo-v10.html con marker dinamici
 */
...
<body>
  ...
  <div class="conditions">
    ...
    <div class="conditions-content">
      <span>Validità: {{preventivo.dataValidita}}</span>
      <span>Pagamento: {{preventivo.metodoPagamento}}</span>
...
async function updateTemplate() {
  try {
    console.log('🔄 Aggiornamento template PREVENTIVO v11...\n');
    ...
    if (!template) {
      console.log('❌ Nessun template PREVENTIVO attivo trovato');
      return;
    }

    console.log(`📄 Template trovato: ${template.id}`);
    console.log(`   Nome: ${template.name}`);
    console.log(`   Content length attuale: ${template.content?.length || 0}`);

    // Crea contenuto nel formato __htmlEditor
    const newContent = JSON.stringify({
      __htmlEditor: true,
      version: '11.0',
      rawHtml: TEMPLATE_HTML,
    ...
    console.log(`   Nuovo content length: ${newContent.length}`);
    console.log(`   Formato: __htmlEditor v11.0`);
    console.log('\n📋 Markers disponibili nel template:');
    console.log('   - {{tenant.logoHtml}} - Logo tenant');
    console.log('   - {{tenant.name}} - Nome tenant');
    console.log('   - {{tenant.address}} - Indirizzo tenant');
    console.log('   - {{tenant.vatNumber}} - P.IVA tenant');
    console.log('   - {{preventivo.numero}} - Numero preventivo');
    console.log('   - {{preventivo.dataEmissione}} - Data emissione');
    console.log('   - {{preventivo.dataValidita}} - Data validità');
    console.log('   - {{preventivo.tipoServizio}} - Tipo servizio');
    console.log('   - {{preventivo.partecipantiHtml}} - Riga partecipanti');
    console.log('   - {{preventivo.metodoPagamento}} - Metodo pagamento');
    console.log('   - {{cliente.nome}} - Nome cliente');
    console.log('   - {{cliente.dettagliHtml}} - Dettagli cliente (P.IVA, CF, indirizzo)');
    console.log('   - {{corso.boxHtml}} - Box servizio/corso');
    console.log('   - {{vociHtml}} - Righe tabella voci');
    console.log('   - {{totaliHtml}} - Righe totali');
    console.log('   - {{noteHtml}} - Box note (condizionale)');
