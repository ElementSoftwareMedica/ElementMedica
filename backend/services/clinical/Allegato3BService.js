...
class Allegato3BService {
    ...
    static async compileStatistics(companyTenantProfileId, anno, tenantId) {
        ...
        // 1. Lavoratori sorvegliati nell'anno
        const lavoratoriSorvegliati = await this.getLavoratoriSorvegliati(companyTenantProfileId, anno, tenantId);
        ...
        return {
            totLavoratoriSorvegliati: lavoratoriSorvegliati.count,
            lavoratoriPerGenere: lavoratoriSorvegliati.perGenere,
            lavoratoriPerFasciaEta: lavoratoriSorvegliati.perFasciaEta,
    ...
    static async getLavoratoriSorvegliati(companyTenantProfileId, anno, tenantId) {
        ...
        // Statistiche per genere (anonimizzate)
        const perGenere = {
            maschi: lavoratori.filter(l => l.paziente?.gender === 'MALE').length,
            femmine: lavoratori.filter(l => l.paziente?.gender === 'FEMALE').length,
            altro: lavoratori.filter(l => !['MALE', 'FEMALE'].includes(l.paziente?.gender)).length
        };

        // Statistiche per fascia età (anonimizzate)
        const oggi = new Date();
        const perFasciaEta = {
            'sotto18': 0,
            '18-25': 0,
    ...
    static async getStatistichePerRischio(companyTenantProfileId, anno, tenantId) {
        // Recupera tutti i rischi delle mansioni aziendali
        ...
        rischi.forEach(r => {
            ...
            statPerRischio[codice].lavoratoriEsposti += numLavoratori;

            if (r.livello && statPerRischio[codice].perLivello[r.livello] !== undefined) {
                statPerRischio[codice].perLivello[r.livello] += numLavoratori;
            }
        });
