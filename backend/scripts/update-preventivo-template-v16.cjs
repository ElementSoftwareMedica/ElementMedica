/**
 * Update PREVENTIVO Template v16
 * Design elegante come test-preventivo-v10.html
 * SINGLE PAGE LAYOUT con gradienti e stile professionale
 */

const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const TEMPLATE_V16 = `<!DOCTYPE html>
<html lang="it">
<head>
<meta charset="UTF-8">
<style>
...
<body>
<div class="header">
<div class="logo-section">{{tenant.nome}}</div>
<div class="company-info">
<div class="company-name">{{tenant.nome}}</div>
{{tenant.indirizzo}}<br>P.IVA: {{tenant.piva}}
</div>
</div>

<div class="doc-title">
<h1>PREVENTIVO</h1>
<div class="doc-meta"><strong>{{numero}}</strong>del {{dataEmissione}}</div>
</div>
...
<p><span class="label">Valido fino:</span> <span class="value">{{dataScadenza}}</span></p>
<p><span class="label">Tipo:</span> <span class="value">{{tipoServizio}}</span></p>
<p><span class="label">Stato:</span> <span class="value">{{stato}}</span></p>
</div>
</div>

<div class="service-box">
<h3>{{titoloServizio}}</h3>
{{corso.metaHtml}}
</div>
...
<span>Validità: {{dataScadenza}}</span>
...
<div class="footer">{{tenant.nome}} - {{tenant.indirizzo}} - P.IVA {{tenant.piva}}</div>
</body>
</html>`;

async function main() {
    console.log('🔄 PREVENTIVO → v16 ELEGANT SINGLE PAGE\n');
    ...
    try {
        ...
        if (existing) {
            ...
            const updated = await prisma.templateLink.update({
                ...
                data: {
                    content: TEMPLATE_V16,
                    version: 16,
                    isActive: true,
                    updatedAt: new Date()
                }
            });
            console.log(`✅ v16 applicato (ID: ${updated.id})`);
            console.log(`   Content length: ${TEMPLATE_V16.length}`);
        } else {
