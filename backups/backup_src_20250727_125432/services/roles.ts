...
class RolesService {
  ...
  async getAll(): Promise<{ roles: Role[], totalRoles: number, totalUsers: number }> {
    const response = await apiGet<{ success: boolean, data: { roles: Role[], totalRoles: number, totalUsers: number } }>(this.baseUrl);
    // Il backend restituisce { success: true, data: { roles: [...], totalRoles: number, totalUsers: number } }
    return response.data;
  }

  async getById(roleType: string): Promise<Role> {
    const encodedRoleType = encodeURIComponent(roleType);
    const response = await apiGet<{ success: boolean, data: Role }>(`${this.baseUrl}/${encodedRoleType}`);
    return response.data;
  }
  ...
  async create(role: { 
    ...
    const response = await apiPost<{ success: boolean, data: Role }>(this.baseUrl, role);
    return response.data;
  }
  ...
  async getRoleHierarchy(): Promise<RoleHierarchy> {
    const response = await apiGet<{ success: boolean, data: { hierarchy: RoleHierarchy } }>(`${this.baseUrl}/hierarchy`);
