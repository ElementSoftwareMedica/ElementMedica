/**
 * Script per aggiornare il template PREVENTIVO alla versione v14
 * USA MARKER COMPOSITI HTML (NON Handlebars)
 * 
 * Il sistema MarkerResolver NON supporta:
 *   {{#if}}, {{#each}}, {{else}}
 * 
 * USA invece i marker compositi generati dal backend:
 *   {{vociHtml}} - righe tabella pre-renderizzate
 *   {{totaliHtml}} - sezione totali pre-renderizzata
 *   {{noteHtml}} - note condizionali pre-renderizzate
 *   {{cliente.dettagliHtml}} - dettagli cliente pre-renderizzati
 *   {{corso.metaHtml}} - meta corso pre-renderizzati
 *   {{preventivo.partecipantiHtml}} - partecipanti pre-renderizzati
 */
const { PrismaClient } = require('@prisma/client');
const prisma = new PrismaClient();

const TEMPLATE_V14 = `<!DOCTYPE html>
<html lang="it">
<head>
  <meta charset="UTF-8">
  <style>
...
<body>
  ...
  <!-- NOTE (condizionale, pre-renderizzato) -->
  {{noteHtml}}
...
async function updateTemplates() {
    console.log('🔄 Aggiornamento template PREVENTIVO a v14 (marker compositi)...\n');
    ...
    try {
        ...
        for (const template of templates) {
            ...
            await prisma.templateLink.update({
                ...
                data: {
                    content: TEMPLATE_V14,
                    version: 14,
                    name: 'Preventivo Elegante V14',
                    updatedAt: new Date()
                }
            });

            console.log('  ✅ Template aggiornato a v14');
        }

        console.log('\n✅ Aggiornamento completato!');
        console.log('\nCaratteristiche v14:');
        console.log('  • USA MARKER COMPOSITI (no Handlebars)');
        console.log('  • {{vociHtml}} per righe tabella');
        console.log('  • {{totaliHtml}} per sezione totali');
        console.log('  • {{noteHtml}} per note condizionali');
        console.log('  • {{cliente.dettagliHtml}} per dettagli cliente');
        console.log('  • {{corso.metaHtml}} per meta corso');
        console.log('  • Font 10pt elegante');
