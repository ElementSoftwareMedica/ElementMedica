 ...
 * 
 * Extracted from AdvancedPermissionsPage for tab-based layout
 * 
 * @module pages/management/permissions/PermissionManagementTab
...
const ROLE_TYPES: Record<string, { name: string; description: string; level: number; color: string; icon: React.FC<{ className?: string }> }> = {
    ...
    ADMIN: {
        name: 'Admin',
        description: 'Amministratore principale',
        level: 2,
    ...
    TENANT_ADMIN: {
        name: 'Tenant Admin',
        description: 'Gestione completa tenant',
        level: 3,
    ...
    COMPANY_ADMIN: {
        name: 'Admin Azienda',
        description: 'Gestione azienda cliente',
        level: 4,
    ...
    TRAINING_ADMIN: {
        ...
        description: 'Gestione formazione',
        level: 4,
...
const PermissionManagementTab: React.FC = () => {
    ...
    // P69: Auto-select role from URL parameter
    useEffect(() => {
        const roleTypeFromUrl = searchParams.get('roleType');
        if (roleTypeFromUrl && roles.length > 0 && !selectedRole) {
            const roleToSelect = roles.find(r => r.type === roleTypeFromUrl);
            if (roleToSelect) {
                selectRole(roleToSelect);
            }
        }
    }, [searchParams, roles, selectedRole, selectRole]);
        ...
        <div className="flex flex-col h-full min-h-0 gap-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center space-x-6">
                    <div className="text-center">
                        <div className="text-2xl font-bold text-violet-600">{roles.length}</div>
                ...
                <div className="flex items-center space-x-2">
                    ...
                    {isChangingRole && (
                        <div className="flex items-center space-x-1 px-2 py-1 bg-violet-100 border border-violet-300 rounded-lg">
                            <Loader2 className="w-3 h-3 border-2 border-violet-600 border-t-transparent rounded-full animate-spin" />
                            <span className="text-xs font-medium text-violet-700">Cambio ruolo...</span>
                        </div>
                    )}
                    <button
                        onClick={() => loadRoles()}
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 rounded-xl p-6">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <h3 className="font-semibold text-violet-900 dark:text-violet-300">Seleziona un ruolo</h3>
                        ...
                        {roles.map((role: Role, index: number) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-violet-400 dark:hover:border-violet-600 hover:shadow-md transition-all text-left group relative ${isChangingRole || !role ? 'opacity-50 cursor-not-allowed' : ''
                                        }`}
                                >
                                    ...
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs text-violet-600 font-medium">{role?.userCount || 0} utenti</span>
                                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${config.color}`}>
                                            {roleType}
                                        </span>
                                    </div>
                                    <div className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-violet-600 to-purple-600 transform scale-x-0 group-hover:scale-x-100 transition-transform rounded-b-xl" />
                                </button>
                            );
                        })}
                    </div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-6">
                        <div className="flex items-start space-x-3">
                            ...
                            <div className="flex-1">
                                <h4 className="font-medium text-blue-900 dark:text-blue-300 mb-3">Guida ai Permessi</h4>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800 dark:text-blue-300">
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs">Crea, Leggi, Modifica, Elimina per ogni entità</p>
                                        </div>
                                    </div>
                                    <div className="space-y-3">
                                        <div>
            ...
            {selectedRole && (
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    {/* OptimizedPermissionManager - P69: Removed wrapper padding to fix scroll */}
                    <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden min-h-0">
