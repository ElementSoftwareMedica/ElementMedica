/**
 * FirmaModal - Firma digitale e upload PDF firmati per un preventivo
 *
 * Due modalità:
 *   - Firma digitale: disegna/carica firma → posiziona sul PDF (SigningWorkflowModal)
 *   - Carica firmato: upload di scansione cartacea
 * Storico: lista di tutti i documenti firmati caricati/generati
 */

import React, { useState, useRef, useEffect } from 'react';
import { X, Upload, FileText, Download, Clock, CheckCircle, Loader2, Pen } from 'lucide-react';
...
export function FirmaModal({ preventivoId, preventivoNumero, isOpen, onClose }: FirmaModalProps) {
  ...
  const handleSigningConfirm = async ({
    signatureDataUrl,
    placement,
  }: {
    ...
    setUploading(true);
    try {
      await apiPost(`/api/v1/preventivi/${preventivoId}/pdf-firmati/firma-digitale`, {
        signatureData: signatureDataUrl,
        placement,
      });
      showToast({ message: 'Firma digitale applicata e salvata con successo', type: 'success' });
      await fetchPdfFirmati();
    } catch {
      showToast({ message: 'Errore nell\'applicazione della firma digitale', type: 'error' });
    } finally {
      setUploading(false);
    }
  };
  ...
  const pdfPreviewUrl = `/api/v1/preventivi/${preventivoId}/pdf`;
  const token = getToken();
  const previewHeaders = token ? { Authorization: `Bearer ${token}` } : undefined;
  ...
  return (
    <>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg">
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
              {([['digitale', 'Firma digitale', Pen], ['upload', 'Carica firmato', Upload]] as const).map(([m, label, Icon]) => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
            ...
            {mode === 'digitale' ? (
              <div className="space-y-3">
                <div className="rounded-lg border border-blue-100 bg-blue-50/60 p-3 text-sm text-blue-800 dark:border-blue-800 dark:bg-blue-900/20 dark:text-blue-200">
                  Disegna o carica la tua firma e posizionala sul preventivo PDF. Il documento firmato sarà salvato nello storico.
                </div>
                <button
                  type="button"
                ...
                >
                  ...
                  {uploading ? 'Applicazione firma…' : 'Apri strumento firma'}
                </button>
              </div>
            ) : (
              <div className="space-y-3">
      ...
      {/* Signing workflow opens as overlay on top */}
      <SigningWorkflowModal
        isOpen={showSigning}
        documentId={preventivoId}
        documentLabel={`Preventivo ${preventivoNumero}`}
        previewUrl={pdfPreviewUrl}
        previewHttpHeaders={previewHeaders}
