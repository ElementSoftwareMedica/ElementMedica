...
/**
 * Verifica limiti utilizzo codice
 * 
 * @param {string} codiceId - ID codice
 * @param {string} clienteId - ID cliente
 * @param {string} clienteType - Tipo cliente
 * @returns {Promise<Object>} Stato limiti
 */
export async function checkCodeLimits(codiceId, clienteId = null, clienteType = null) {
  try {
    const codice = await prisma.codiceSconto.findUnique({
      where: { id: codiceId },
      select: {
        utilizzoMassimo: true,
        utilizzoCorrente: true,
        utilizzoPerUtente: true
      }
    });
    ...
    const hasGlobalLimit = codice.utilizzoMassimo !== null;
    const globalLimitReached = hasGlobalLimit && codice.utilizzoCorrente >= codice.utilizzoMassimo;
    ...
    if (codice.utilizzoPerUtente && clienteId) {
      hasUserLimit = true;
      userUsageCount = await prisma.preventivoSconto.count({
        where: {
          codiceId,
          preventivo: {
            OR: [
      ...
      userLimitReached = userUsageCount >= codice.utilizzoPerUtente;
    }
    
    return {
      globalLimit: {
        hasLimit: hasGlobalLimit,
        current: codice.utilizzoCorrente,
        max: codice.utilizzoMassimo,
        reached: globalLimitReached,
        remaining: hasGlobalLimit ? Math.max(0, codice.utilizzoMassimo - codice.utilizzoCorrente) : null
      },
      userLimit: {
        hasLimit: hasUserLimit,
        current: userUsageCount,
        max: codice.utilizzoPerUtente,
        reached: userLimitReached,
        remaining: hasUserLimit ? Math.max(0, codice.utilizzoPerUtente - userUsageCount) : null
      },
      canUse: !globalLimitReached && !userLimitReached
    };
    
  } catch (error) {
    logger.error('Error checking code limits', {
      component: 'codici-sconto-service',
      function: 'checkCodeLimits',
      codiceId,
      error: error.message
    });
    throw error;
 ...
 * 
 * @param {string} codiceId - ID codice
 * @param {Object} transaction - Prisma transaction client (opzionale)
 * @returns {Promise<Object>} Codice aggiornato
 */
export async function incrementCodeUsage(codiceId, transaction = null) {
  const client = transaction || prisma;
  
  try {
    return await client.codiceSconto.update({
      where: { id: codiceId },
      data: {
        utilizzoCorrente: {
          increment: 1
    ...
    logger.error('Error incrementing code usage', {
      component: 'codici-sconto-service',
      function: 'incrementCodeUsage',
      codiceId,
      error: error.message
    });
    throw error;
...
/**
 * Decrementa contatore utilizzo codice (quando si rimuove uno sconto)
 * 
 * @param {string} codiceId - ID codice
 * @param {Object} transaction - Prisma transaction client (opzionale)
 * @returns {Promise<Object>} Codice aggiornato
 */
export async function decrementCodeUsage(codiceId, transaction = null) {
  const client = transaction || prisma;
  ...
  try {
    ...
    logger.error('Error decrementing code usage', {
      component: 'codici-sconto-service',
      function: 'decrementCodeUsage',
      codiceId,
      error: error.message
    });
    throw error;
